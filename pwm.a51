...
; La diff?renciation se fait au niveau de la valeur de R0 (utilisA(Š en adressage indirect)
...
; R?guli?rement, la valeur de ce timer est v?rifi?e. La d?cision d'augmenter la vitesse ou de la diminuer est alors prise selon cette valeur. Enfin, le timer est remis ? zero.
            ...
            ; RA(Šveil par cette interruption (mode idle)
            org 000Bh
            clr TF0
            reti
...
init:
            ...
            mov VITESSE_NORMALE, #160
            mov VITESSE, VITESSE_NORMALE
            mov ATTENTE_ASSERV, #20     ;2.5 * 20 = 500ms entre deux changements de puissance d?livr?e au moteur

            ; TIMER
            mov TMOD, #01010001b     ; initialisation du timer 0 (mode 16bits) et du timer 1 (mode 16bits, avec horloge externe) et dA(Šmarrage
            setb TR0
            setb TR1
            ...
            nop    ; A(Šquilibrage de timer 0 (il n'en faut qu'un)
; ne rien A(Šcrire ici
...
etat_bas:
            clr PIN_MOTEUR
            nop                                ; afin que l'A(Šcart temporel entre les deux clr soit le me;me qu'en les deux mov
            nop
            nop
            nop
            nop
...
est_nul:
            mov NOIR_DROIT, C
fin_xor:
                ; ici, C vaut 1 si on a détecté la bande noire de l'autre capteur (comparé à la dernière fois où on l'a vu)
            jnc fin_direction
tourne:
