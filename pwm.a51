...
; La diff?renciation se fait au niveau de la valeur de R0 (utilis√© en adressage indirect)
...
; R1: pointe vers un octet entre 60h et 6Fh. Sert ‡ la mÈmorisation des derniËres valeurs.
...
; R?guli?rement, la valeur de ce timer est v?rifi?e. La d?cision d'augmenter la vitesse ou de la diminuer est alors prise selon cette valeur. Enfin, le timer est remis ? zero.
            ...
            CAPT_G            bit    P1.7
            DIODE             bit    P1.0
            LASER_ACTIVE      bit      P3.0    ; modifi√© par la carte esclave
...
init:
            ...
            mov DROITITUDE, #0	; ‡ vÈrifier
            mov BP_MEM, #0FFh
            mov 60h, #0FFh
            mov COMPTEUR, #8	; afin de ne pas avoir un coup de boost au dÈpart
        
;mov VITESSE, #0    
;mov DIRECTION, #35    ; pour test
        
            ; TIMER
            mov TMOD, #01010001b     ; initialisation du timer 0 (mode 16bits) et du timer 1 (mode 16bits, avec horloge externe) et d√©marrage
            setb TR0
            setb TR1
            ...
            nop    ; √©quilibrage de timer 0 ("par chance", il n'en faut qu'un)
; ne rien √©crire ici
...
etat_bas:
            clr PIN_MOTEUR
            nop                                ; afin que l'√©cart temporel entre les deux clr soit le me;me qu'en les deux mov
            nop
            nop
            nop
            nop
...
; gestion des capteurs et de la direction
                    ; cas particulier: si les deux d√©tectent en me;me temps, on va tout droit
...
passageLigneNoire:
                djnz NB_TOURS, tout_droit   ; on compte le nombre de tours. Le d√©but d'un tour est rep√©r√© par la bande noire.
                ; POWER DOWN
                orl PCON, #10b
                jmp fin_direction		; le cas de la ligne noire est un peu complexe du fait que NOIR_DROIT est binaire et ne peut gÈrer ce cas.
pasNouveauTour:
...
fin_xor:
            jnc pasChange
            clr PANIQUE     ; ouf, on a retrouv√© la ligne
...
tourne_droite:
                ...
                clr C
                subb A, #50    ; la diff√©rence doit faire 28
pas_braque_droite:             
;                add A, DROITITUDE
                mov DIRECTION, A
fin_direction:
...
; asservissement vitesse et adoucissement rotation
      ...
      mov A, @R1
      jnb ACC.0, pas_diminuer_compteur    ; ACC.0 est le bit qui va dispara√Ætre
      dec COMPTEUR
...
pas_augmenter_compteur:
      mov ACC.0, C                 ; une valeur est conserv√©e 2560ms
      rlc A
      mov ACC.0, C
      mov @R1, A
...
fin_consigne:
      ...
      mov A, TL1			; on rÈcupËre le nombre de fragments de tours de roues, qu'on rÈinitialise au passage
      mov TL1, #0
      clr C
      subb A, #3d
